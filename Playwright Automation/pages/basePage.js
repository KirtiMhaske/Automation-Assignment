...
class BasePage {
	...
	async waitAndClick1() {
		return await this.page.getByRole('button', { name: 'Proceed to checkout' }).click()
	}

	async checkCheckbox(selector) {
		return await this.page.locator(selector).check()
	}
	...
	async getFirstElementFromTheList(selector, text) {
		const rows = await this.page.locator(selector)
		const firstItem = await rows.nth(0).click()
		return firstItem
	}
	...
	async getSpecifiedElementFromTheList(selector, text) {
		const row = await this.page.getByRole("listitem").filter({ hasText: text })
